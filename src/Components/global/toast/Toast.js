...
const Toast = () => {
  ...
  const getMessageWidth = m => {
    if (m instanceof Array) return 'w-auto';
  ...
  const determineVariantClasses = () => {
    ...
    if (variant === 'warning') return 'text-black-dark bg-warning';
  ...
  const renderMessage = () => {
    ...
    if (message instanceof Array) {
      return message.map((message, index) => (
        <p className='my-0.5' key={index}>{`${message}`}</p>
      ));
    }
  ...
  return (
    <SlideAnimation slideIn={show} direction='left' timeout={150}>
      <div
        //prettier-ignore
        className={`fixed z-50 top-11 shadow-2xl right-5 ${determineVariantClasses()} ${getMessageWidth(message)} max-w-xs max-h-full  justify-between flex items-center px-2 py-0.5`}
      >
        <div className='text-xs tracking-tight px-2 py-1 max-h-28 overflow-scroll break-words w-11/12'>
          {renderMessage()}
        </div>
